'use client';
// S18 — Manage Locations  (src/app/admin/locations/page.js)
...
export default function LocationsPage() {
    ...
    function isOccupied(loc) {
        const batches = loc.inventory_batches;
        return (batches?.reduce((s, b) => s + b.quantity_available, 0) ?? 0) > 0;
    }
    ...
    async function toggleActive(id, currentActive, occupied) {
        if (occupied && currentActive) {
            alert('Cannot deactivate an occupied location.');
            return;
        }
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 16 }}>
        ...
        <div>
          ...
          <div className="card">
            ...
            return (<tr key={loc.id} style={{ opacity: loc.is_active ? 1 : 0.5 }}>
                    ...
                    : <span className="badge badge-gray">Inactive</span>}
                      ...
                      <td className="table-td">
                        <button onClick={() => toggleActive(loc.id, loc.is_active, occupied)} className="btn btn-sm btn-ghost" disabled={occupied && loc.is_active}>
                          {loc.is_active ? 'Deactivate' : 'Reactivate'}
                        </button>
                      </td>
                    </tr>);
        })}
                {!filtered.length && (<tr><td colSpan={7} className="table-td" style={{ textAlign: 'center', padding: 32 }}>No locations yet</td></tr>)}
              </tbody>
            </table>
          ...
          <div className="card card-pad">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 12 }}>
              <div>
                ...
                <input type="text" className="input" maxLength={2} placeholder="A" value={zone} onChange={e => setZone(e.target.value.toUpperCase())}/>
              </div>
              <div>
                <label className="section-label">Rack</label>
                <input type="number" className="input" min="1" max="99" placeholder="05" value={rack} onChange={e => setRack(e.target.value)}/>
              </div>
              <div>
                <label className="section-label">Slot</label>
                <input type="number" className="input" min="1" max="99" placeholder="01" value={slot} onChange={e => setSlot(e.target.value)}/>
              </div>
            </div>
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              <input type="text" className="input" placeholder="e.g. Near loading bay" value={description} onChange={e => setDescription(e.target.value)}/>
            </div>

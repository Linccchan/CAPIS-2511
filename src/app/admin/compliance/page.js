// S10 — Compliance & Stickers  (src/app/admin/compliance/page.js)
'use client';
...
export default function CompliancePage() {
    ...
    useEffect(() => { load(); }, [load]);
    async function advanceDesign(id, currentStatus) {
        const next = {
    ...
    const filteredDesigns = filterOrder ? designs.filter(d => d.customer_orders?.order_number === filterOrder) : designs;
    const filteredLabeling = filterOrder ? labelingTasks.filter(t => t.customer_orders?.order_number === filterOrder) : labelingTasks;
    return (<div style={{ maxWidth: 900 }}>
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            ...
            const order = d.customer_orders;
            return (<tr key={d.id}>
                  <td className="table-td"><span className="td-primary">{order?.order_number}</span></td>
                  <td className="table-td">{d.products?.product_name}</td>
                  ...
                  <td className="table-td">
                    {d.status !== 'printed' && (<button onClick={() => advanceDesign(d.id, d.status)} disabled={updating === d.id} className="btn btn-sm btn-ghost">
                    ...
                    {d.status === 'awaiting_customer' && (<button className="btn btn-sm btn-secondary" style={{ marginLeft: 6 }}>Resend photos</button>)}
                  </td>
                </tr>);
        })}
            {!filteredDesigns.length && (<tr><td colSpan={6} className="table-td" style={{ textAlign: 'center', padding: 32 }}>No sticker designs</td></tr>)}
          </tbody>
        </table>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
        ...
        {totalLabels > 0 && (<div style={{ fontSize: 11, color: 'var(--text-secondary)' }}>
      ...
      <div className="card">
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {filteredLabeling.map(t => (<tr key={t.id}>
                <td className="table-td"><span className="td-primary">{t.customer_orders?.order_number}</span></td>
                <td className="table-td">{t.products?.product_name}</td>
                ...
                <td className="table-td">{t.profiles?.full_name ?? <span style={{ color: 'var(--text-tertiary)' }}>Unassigned</span>}</td>
                <td className="table-td"><StatusBadge status={t.status}/></td>
              </tr>))}
            {!filteredLabeling.length && (<tr><td colSpan={7} className="table-td" style={{ textAlign: 'center', padding: 32 }}>No labeling tasks</td></tr>)}
          </tbody>
        </table>
        <p className="info-note" style={{ margin: 12 }}>

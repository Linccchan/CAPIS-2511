'use client';
// S30 — Supplier Cost Management  (src/app/admin/supplier-costs/page.js)
import { useEffect, useMemo, useState } from 'react';
import { createClient } from '@/lib/supabase/client';
export default function SupplierCostsPage() {
    const supabase = useMemo(() => createClient(), []);
    const [costPrompts, setCostPrompts] = useState([]);
    ...
    async function handleDecision(notifId, productId, supplierId, action, impliedCost) {
        const { data: { user } } = await supabase.auth.getUser();
    ...
    return (<div style={{ maxWidth: 900 }}>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
        <input type="text" className="input" placeholder="Filter by product…" style={{ width: 200 }} value={filter} onChange={e => setFilter(e.target.value)}/>
      ...
      {/* Cost update prompts */}
      {costPrompts.length > 0 && (<div style={{ marginBottom: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
          {costPrompts.map(n => {
                ...
                return (<div key={n.id} className="alert-banner alert-warn">
                <div style={{ flex: 1 }}>
                          ...
                          <td className="table-td">
                            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                              ...
                              <div style={{ display: 'flex', gap: 4 }}>
                                <input type="number" className="input" style={{ width: 80 }} placeholder="Custom…" value={customValues[n.id] ?? ''} onChange={e => setCustomValues(prev => ({ ...prev, [n.id]: e.target.value }))}/>
      ...
      <div className="card">
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {filteredCosts.map(c => (<tr key={c.id}>
                <td className="table-td">
            ...
            {!filteredCosts.length && (<tr><td colSpan={7} className="table-td" style={{ textAlign: 'center', padding: 32 }}>No cost records yet</td></tr>)}
          </tbody>
        </table>
      </div>
    </div>);

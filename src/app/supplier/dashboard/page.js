...
export default function SupplierDashboard() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (supplierData?.id) {
        const [{ data: poData }, { data: notificationData }] = await Promise.all([
          supabase
  ...
  if (loading) {
    return <div className="card card-pad" style={{ maxWidth: 480 }}>Loading supplier dashboard...</div>
  }
  ...
  const overdueCount = openPOs.filter((po) => po.expected_delivery_date && po.expected_delivery_date < today).length
  ...
  return (
    <div style={{ maxWidth: 900 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 24 }}>
        {[
          { val: openPOs.length, lbl: 'Open purchase orders', sub: overdueCount > 0 ? `${overdueCount} overdue` : 'None overdue' },
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 16 }}>
        <div className="card">
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              ...
              {openPOs.length === 0 && <tr><td colSpan={6} className="table-td" style={{ textAlign: 'center', padding: 32 }}>No open purchase orders</td></tr>}
            </tbody>
          </table>
        </div>
        ...
        <div className="card">
          ...
          {notifications.map((notification) => (
            <div key={notification.id} style={{ padding: '10px 14px', borderBottom: '1px solid var(--bg)', fontSize: 11 }}>
              <div style={{ fontWeight: 500, color: 'var(--text-primary)' }}>{notification.title}</div>
              {notification.message && <div style={{ color: 'var(--text-tertiary)', marginTop: 2 }}>{notification.message}</div>}
            </div>
          ))}
          {notifications.length === 0 && <div style={{ padding: '16px 14px', fontSize: 11, color: 'var(--text-tertiary)' }}>No new notifications</div>}
        </div>
      </div>
    </div>
  )

'use client';
// S21 — Log Dispatch  (src/app/supplier/purchase-orders/[id]/dispatch/page.tsx)
...
export default function LogDispatchPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setPO(data);
      setItems((data?.purchase_order_items ?? []).map((i: any) => ({
        ...i,
  ...
  async function handleSubmit() {
    ...
    if (error || !delivery) { setSaving(false); return; }
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  return (
                    <tr key={item.id}>
                      ...
                      <td className="table-td" style={{ textAlign: 'right' }}>
                        {item.fulfilled && (
                          <input type="number" min="0" max={remaining} className="input" style={{ width: 80, textAlign: 'right' }}
                            value={item.dispatch_qty}
                            onChange={e => updateItem(item.id, 'dispatch_qty', e.target.value)} />
                        )}
                      </td>
                      <td className="table-td">
                        {item.fulfilled && Number(item.dispatch_qty) < remaining && (
                          <input type="text" className="input" placeholder="Short supply reason…"
                            value={item.partial_reason}
                            onChange={e => updateItem(item.id, 'partial_reason', e.target.value)} />
                        )}
                      </td>
                      <td className="table-td">
                        <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
                          <input type="checkbox" checked={item.fulfilled}
                            onChange={e => updateItem(item.id, 'fulfilled', e.target.checked)} />
          ...
          <div className="card card-pad">
              ...
              <div>
                ...
                <div style={{ border: '1px dashed var(--border)', borderRadius: 'var(--radius)', padding: '10px 12px', background: 'var(--bg)', fontSize: 11, color: 'var(--text-tertiary)' }}>
                  <input type="file" accept=".pdf,.jpg,.jpeg,.png"
                    ...
                    style={{ display: 'block', fontSize: 11, width: '100%' }} />
            ...
            <div>
              ...
              <textarea className="input" rows={3} placeholder="Any notes about this dispatch…"
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div className="card card-pad">
            ...
            <div style={{ fontSize: 11, display: 'flex', flexDirection: 'column', gap: 6 }}>
              ...
              {isPartial && (
                <div className="alert-banner alert-warn" style={{ marginTop: 8, padding: '8px 10px', fontSize: 11 }}>
          ...
          <p style={{ fontSize: 10, color: 'var(--text-tertiary)', textAlign: 'center' }}>
            Warehouse will confirm receipt. You'll be notified once done.
          </p>

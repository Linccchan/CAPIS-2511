'use client';
// S16 — Staging Tracker  (src/app/warehouse/staging/page.tsx)
...
export default function StagingPage() {
  ...
  useEffect(() => {
    async function loadOrders() {
      const { data } = await supabase.from('customer_orders')
        ...
        .order('order_number');
      setOrders(data ?? []);
      if (data?.length) setSelectedOrderId(data[0].id);
    }
    loadOrders();
  }, []);
  ...
  async function updateStaged(id: string, qty: number) {
    await supabase.from('staging_tasks').update({ staged_quantity: qty }).eq('id', id);
  }

  async function markComplete(taskId: string, requiredQty: number) {
    setSaving(true);
  ...
  function getStickerStatus(productId: string) {
    ...
    <div style={{ maxWidth: 900 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <select className="input" style={{ width: 200 }} value={selectedOrderId}
            onChange={e => setSelectedOrderId(e.target.value)}>
            {orders.map(o => (
              <option key={o.id} value={o.id}>
                {o.order_number} — {(o.customers as any)?.company_name}
              </option>
            ))}
          </select>
        </div>
        ...
        <>
          ...
          <div className="card card-pad" style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              {allStaged && allStickered && selectedOrder.status === 'warehouse_preparation' && (
                <button onClick={markReadyToShip} disabled={saving} className="btn btn-primary">
          ...
          <div className="card">
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                  ...
                  return (
                    <tr key={t.id}>
                      <td className="table-td">
                        <div className="td-primary">{(t.products as any)?.product_name}</div>
                        <div style={{ fontSize: 10, fontFamily: 'monospace', color: 'var(--text-tertiary)' }}>{(t.products as any)?.sku}</div>
                      </td>
                      <td className="table-td">
                        <span style={{ fontFamily: 'monospace', fontSize: 11, background: 'var(--bg)', padding: '2px 6px', borderRadius: 4 }}>
                          {(t.warehouse_locations as any)?.location_code ?? '—'}
                        </span>
                      </td>
                      <td className="table-td" style={{ textAlign: 'right' }}>
                        {staged ? (
                          <span style={{ fontWeight: 500 }}>{t.staged_quantity} {(t.products as any)?.unit}</span>
                        ) : (
                          <input type="number" min="0" max={t.required_quantity} className="input" style={{ width: 70, textAlign: 'right' }}
                            defaultValue={t.staged_quantity}
                            onBlur={e => updateStaged(t.id, Number(e.target.value))} />
                        )}
                      </td>
                      ...
                      <td className="table-td">
                        {staged
                          ? <span style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>{(t.profiles as any)?.full_name} · {t.completed_at ? new Date(t.completed_at).toLocaleDateString('en-PH', { month: 'short', day: 'numeric' }) : '—'}</span>
                          : '—'
                        }
                      </td>
                      <td className="table-td">
                        {!staged && (
                          <button onClick={() => markComplete(t.id, t.required_quantity)} disabled={saving} className="btn btn-sm btn-primary">
                            Mark staged
                          </button>
                        )}
                      </td>
                ...
                {!tasks.length && (
                  <tr><td colSpan={7} className="table-td" style={{ textAlign: 'center', padding: 32 }}>No staging tasks for this order</td></tr>
                )}
              </tbody>
            </table>
            {allStaged && allStickered && (
              <div style={{ padding: '12px 16px', background: '#f0fdf4', borderTop: '1px solid #bbf7d0', fontSize: 12, color: '#15803d', fontWeight: 500 }}>

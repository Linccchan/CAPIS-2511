'use client';
// S14 — Log Delivery / Confirm Receipt  (src/app/warehouse/log-delivery/[poId]/page.tsx)
import { useEffect, useState } from 'react';
import { useParams, useRouter } from 'next/navigation';
import { createClient } from '@/lib/supabase/client';
import Link from 'next/link';

type Condition = 'good' | 'damaged' | 'missing' | 'wrong_item';
...
export default function LogDeliveryPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const available = (locs ?? []).filter(l => {
        const batches = l.inventory_batches as any[];
        const totalQty = batches?.reduce((s: number, b: any) => s + b.quantity_available, 0) ?? 0;
        return totalQty === 0;
      });
  ...
  async function handleConfirm() {
    ...
    for (const item of items) {
      ...
      if (actualQty > 0) {
        await supabase.from('inventory_batches').insert({
          order_id: (po.customer_orders as any)?.id,
    ...
    const { data: poItems } = await supabase.from('purchase_order_items')
      .select('quantity_ordered, quantity_received').eq('purchase_order_id', params.poId as string);
  ...
  if (!po) return <div style={{ padding: 24, color: 'var(--text-tertiary)' }}>Loading…</div>;
  if (!delivery) return (
    <div>
    ...
    <div style={{ maxWidth: 900 }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 2 }}>
          {po.po_number} · {(po.suppliers as any)?.supplier_name} · For {order?.order_number} · Supplier dispatched {delivery.delivery_date}
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 16 }}>
              ...
              <tbody>
                {items.map(item => (
                  <tr key={item.id}>
                    ...
                    <td className="table-td">
                      <select className="input" style={{ width: 130 }} value={item.condition}
              ...
              <tbody>
                {items.map(item => (
                  <tr key={item.id}>
                    ...
                    <td className="table-td">
                      <select className="input" style={{ width: 180 }} value={item.location_id}
                        onChange={e => updateItem(item.id, 'location_id', e.target.value)}>
                        <option value="">— Select location —</option>
                        {locations.map(loc => (
                          <option key={loc.id} value={loc.id}>{loc.location_code} · empty</option>
                        ))}
                      </select>
                    </td>
                  </tr>
        ...
        <div className="card card-pad" style={{ height: 'fit-content' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
            {[
              ['Supplier', (po.suppliers as any)?.supplier_name],
              ['PO #', po.po_number],
              ['Order', order?.order_number],
              ['Dispatched', delivery.delivery_date],
              ['Confirming', new Date().toISOString().split('T')[0]],
            ].map(([l, v]) => (
              <div key={l as string} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid var(--bg)', fontSize: 11 }}>
          ...
          <div style={{ marginTop: 12 }}>
            ...
            <textarea className="input" rows={3} placeholder="Any notes about this receipt…"
